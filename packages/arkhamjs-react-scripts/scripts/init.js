...
module.exports = (appPath, appName, verbose, originalDirectory, template) => {
  ...
  fs.writeFileSync(path.join(appPath, 'package.json'), JSON.stringify(updatedAppPackage, null, 2));
  ...
  if (useYarn) {
    ...
    command = 'npm';
    args = ['install', '--save', verbose && '--verbose'].filter(error => error);
  }
